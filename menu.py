"""
lumoss — Menu Utama (v7.2.11 MEDIA GARDEN — TREE VIEW)
Menu interaktif utama dengan tampilan tree view ala "tree command".

Changelog v7.2.11:
- NEW: Tree view (render_tree_group) — ganti render_vertical_group.
- NEW: Nested tree buat AKUN & RINGKASAN (AKUN AKTIF + RINGKASAN).
- FIX: PR-1 — Layout Main Menu berantakan (SOLVED with tree view).
- FIX: PR-3 — Emoji gear ⚙ nyempil (SOLVED with consistent symbol).
- FIX: PR-5 — templates/about.html cleanup (udah gak dipake).
- KEEP: Semua fungsi lain (upload, regenerate, check, dll).
...
Changelog v7.2.3:
- UI: Main menu full-width + border utuh
- FIX: Layout gak lagi hardcoded width 62

Changelog v7.2.2:
- BREAKING: Output sekarang di output/<slug>/
- BREAKING: Multi-folder media (media_dirs array)
- NEW: Onboarding WAJIB pilih folder media
"""
...
# ═══════════════════════════════════════════════════════════
# MAIN MENU — LAYOUT (v7.2.11 — TREE VIEW)
# ═══════════════════════════════════════════════════════════

def _render_main_menu_v2(callbacks):
    """Render main menu layout (v7.2.11 — tree view)."""
    ...
    # ── Box 1: AKUN & RINGKASAN (nested tree) ──
    _render_top_boxes(slug, info, stats, cfg, theme)
    print()

    # ── Box 2: MENU UTAMA (tree group) ──
    render_tree_group(
        root_label="lumoss/",
        groups=[
            {
    ...
    # Footer — tree style
    print(f"  {C_MOSS_1}└──{C_RESET}  {C_MOSS_1}{C_BOLD}[99]{C_RESET}  {C_RED}✗{C_RESET}  {C_WHITE}Keluar{C_RESET}")
    print()


def _render_menu_banner(slug):
    """Banner main menu (v7.2.11 — pake 🌿)."""
...
def _render_top_boxes(slug, info, stats, cfg, theme):
    """Akun Aktif + Ringkasan — nested tree view (v7.2.11).
    ...
    # Safety net: convert emoji → unicode (kecuali 🌿)
    ...
    # ── Render nested tree ──
    # Root
    print(f"  {C_MOSS_1}{C_BOLD}lumoss/{C_RESET}")
    print(f"  {C_MOSS_1}│{C_RESET}")

    # Section 1: AKUN & RINGKASAN (nested)
    print(f"  {C_MOSS_1}└──{C_RESET}  {C_MOSS_2}{C_BOLD}◉  AKUN & RINGKASAN{C_RESET}")
    print(f"  {C_MOSS_1}    │{C_RESET}")

    # Sub-section 1: AKUN AKTIF
    print(f"  {C_MOSS_1}    ├──{C_RESET}  {C_MOSS_2}{C_BOLD}●  AKUN AKTIF{C_RESET}")
    ...
    for i, line in enumerate(akun_items):
        is_last = (i == len(akun_items) - 1)
        branch = "└──" if is_last else "├──"
        print(f"  {C_MOSS_1}    │   {branch}{C_RESET}  {line}")

    print(f"  {C_MOSS_1}    │{C_RESET}")

    # Sub-section 2: RINGKASAN
    print(f"  {C_MOSS_1}    └──{C_RESET}  {C_MOSS_2}{C_BOLD}▤  RINGKASAN{C_RESET}")
    ...
    for i, line in enumerate(ringkasan_items):
        is_last = (i == len(ringkasan_items) - 1)
        branch = "└──" if is_last else "├──"
        print(f"  {C_MOSS_1}        {branch}{C_RESET}  {line}")
...
def _menu_about():
    ...
    kv_line("Version",  PROJECT_VERSION, value_color=C_GREEN)
...
def _help_structure():
    ...
    print(f"""  {C_MOSS_1}Struktur project Lumoss (v7.2.11):{C_RESET}

  {C_SILVER}/storage/emulated/0/Project/lumoss/{C_RESET}
  │
  ├── ▪ lumoss.py             {C_SILVER}# Entry point{C_RESET}
  ├── ▪ menu.py               {C_SILVER}# Menu utama (tree view){C_RESET}
